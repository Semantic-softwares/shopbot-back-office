...
`+this.COMMANDS.CUT_PAPER;let r=this.storeStore.selectedStore()?.posSettings?.receiptSettings,n=r?.showNote??!1,T=r?.showTax??!0,C=r?.showStoreDetails??!0,l=r?.showSellerInfo??!0,h=r?.showCustomerName??!0,p=r?.footerMessage||"Thank you for your patronage",D=r?.disclaimer||"",t="";if(t+=this.COMMANDS.INIT,C){t+=this.COMMANDS.ALIGN_CENTER,t+=this.COMMANDS.TEXT_DOUBLE_SIZE,t+=this.COMMANDS.BOLD_ON;let o=e.store;o&&o.name&&(t+=`${o.name}
`),t+=`KITCHEN ORDER
...
`)}if(l&&e.staff){let o=e.staff,b=o.name||o.firstName||o;t+=`Server: ${b}
`}t+=`================================
`;let y=e.cart?.products||[],i=this.storeStore.selectedStore()?.currencyCode||"NGN";y.length>0?(t+=this.COMMANDS.BOLD_ON,t+=`ITEMS:
`,t+=this.COMMANDS.BOLD_OFF,y.forEach(o=>{let b=o.name,L=o.quantity||1,E=o.price||0;t+=`${L}x ${b}`,E>0&&(t+=` - ${this.formatCurrency(E,i)}`),t+=`
...
`})):t+=`No items in order
`,t+=`================================
`;let A=e.subTotal||e.subtotal,N=e.tax,g=e.discount,m=e.shippingFee,S=e.serviceFee,M=e.total;return t+=this.COMMANDS.BOLD_ON,t+=`ORDER SUMMARY:
`,t+=this.COMMANDS.BOLD_OFF,A!==void 0&&(t+=`Subtotal:            ${this.formatCurrency(A,i)}
`),T&&N&&N>0&&(t+=`Tax:                 ${this.formatCurrency(N,i)}
`),g&&g>0&&(t+=`Discount:           -${this.formatCurrency(g,i)}
`),m&&m>0&&(t+=`Shipping:            ${this.formatCurrency(m,i)}
`),S&&S>0&&(t+=`Service Fee:         ${this.formatCurrency(S,i)}
`),M!==void 0&&(t+=`--------------------------------
