...
`);let o=e.guestDetails?.totalAdults||1,l=e.guestDetails?.totalChildren||0;if(t+=this.justifyLine("Occupancy:",`${o} Adults, ${l} Children`,i)+`
`,t+=`
...
`,t+=this.COMMANDS.BOLD_OFF,t+=this.getSeparatorLine(i,"-")+`
`,e.rooms&&e.rooms.length>0&&e.rooms.forEach((a,C)=>{let h=a.room||{},A=h.roomNumber||`Room ${C+1}`,u=typeof h.roomType=="object"?h.roomType?.name:h.roomType||"Standard",S=h.priceOverride||h.rate||0;t+=`${A} (${h.name||u})
`,t+=this.justifyLine("Type:",u,i)+`
...
`),e.pricing.fees&&Object.entries(e.pricing.fees).forEach(([A,u])=>{if(u){let S=A.replace(/([A-Z])/g," $1").replace(/^./,M=>M.toUpperCase());t+=this.justifyLine(S+":",this.formatCurrency(u,r?.currency),i)+`
`}}),e.pricing.discounts?.amount&&(t+=this.justifyLine("Discount:","-"+this.formatCurrency(e.pricing.discounts.amount,r?.currency),i)+`
`),t+=this.getSeparatorLine(i,"-")+`
...
`;let a=e.pricing.balance||0,C=a>0?"BALANCE DUE:":"OVERPAID:",h=Math.abs(a);a!==0&&(t+=this.COMMANDS.BOLD_ON,t+=this.justifyLine(C,this.formatCurrency(h,r?.currency),i)+`
