...
export class ReservationFormComponent implements OnDestroy {
  ...
  private populateRooms(rooms: any[]) {
    ...
    rooms.forEach((roomData, index) => {
      ...
      const roomId =
        typeof roomData.room === 'string' ? roomData.room : roomData.room._id;
  ...
  private calculateTotal() {
    ...
    const directDiscountControl = this.reservationForm.get(
    ...
    const discounts =
      this.reservationForm.get('pricing.discounts.amount')?.value || 0;
    ...
    selectedRooms.forEach((roomSelection: any) => {
      if (roomSelection.roomId) {
        const room = availableRooms.find(
          (r) => r._id === roomSelection.roomId || r.id === roomSelection.roomId
        );
        if (room) {
  ...
  private updateGuestCountValidation() {
    const totalAdultsControl = this.reservationForm.get(
      'guestDetails.totalAdults'
    );
    const totalChildrenControl = this.reservationForm.get(
      'guestDetails.totalChildren'
    );
  ...
  isRoomSelected(room: AvailableRoom): boolean {
    ...
    const selectedRoomIds = this.rooms.controls.map(
      (control) => control.get('roomId')?.value
    );
  ...
  toggleRoomSelection(room: AvailableRoom): void {
    ...
    const roomId = room._id || room.id;
    const existingIndex = this.rooms.controls.findIndex(
      (control) => control.get('roomId')?.value === roomId
    );
  ...
  private updateStepValidation(): void {
    ...
    this.reservationDetailsGroup
      .get('roomsSelected')
      ?.setValue(hasRoomsSelected);
  }
  ...
  get additionalGuests(): FormArray {
    return this.reservationForm.get(
      'guestDetails.additionalGuests'
    ) as FormArray;
  }

  // Getter methods for step components
  getReservationDetailsGroup(): FormGroup {
    return this.reservationForm;
  }

  getGuestDetailsGroup(): FormGroup {
    return this.reservationForm.get('guestDetails') as FormGroup;
  }
  ...
  canAddMoreAdditionalGuests(): boolean {
    ...
    const currentTotalAdults =
      this.reservationForm.get('guestDetails.totalAdults')?.value || 1;
    const currentAdditionalGuests = this.additionalGuests.length;
  ...
  onSubmit() {
    if (this.reservationForm.valid) {
      ...
      if (
        !formData.guestDetails?.primaryGuest?.firstName ||
  ...
  private handleGuestAndCreateReservation(formData: any) {
    ...
    if (selectedGuest?._id) {
      ...
      const reservationData = this.transformFormToReservationDto(
        formData,
        selectedGuest._id
      );
  ...
  private transformFormToReservationDto(
    formData: any,
    guestId: string
  ): CreateReservationDto {
    ...
    return {
      ...
      rooms: formData.rooms.map((room: any) => ({
        ...
        guests: {
          ...
          children:
            room.guests?.children || formData.guestDetails.totalChildren || 0,
        },
      })),
  ...
  private setupGuestAutocomplete() {
    this.guestSearchControl.valueChanges.pipe(
      startWith(''),
      debounceTime(300),
      distinctUntilChanged(),
  ...
  private populateGuestDetails(guest: Guest) {
    ...
    this.snackBar.open(
      `Guest ${guest.firstName} ${guest.lastName} selected`,
      'Close',
      { duration: 3000 }
    );
  }
